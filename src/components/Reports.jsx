    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
          <div className="flex justify-between items-start mb-4">
            <div>
              <p className="text-slate-500 text-sm">Taxa de Conclusão</p>
              <h3 className="text-2xl font-bold text-slate-800">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
          <div className="flex items-center justify-between mb-8">
            <h3 className="font-bold text-slate-800 text-lg">Receita Semestral</h3>
          ...
          <div className="flex items-end justify-between h-64 gap-2 pt-4 px-2">
            {monthlyData.map((data, idx) => (
              <div key={idx} className="flex flex-col items-center gap-2 flex-1 group">
                <div className="opacity-0 group-hover:opacity-100 transition-opacity text-xs font-bold text-slate-600 mb-1">
                  R${(data.value / 1000).toFixed(1)}k
                </div>
                <div 
                  className={`w-full max-w-[40px] rounded-t-lg transition-all duration-500 hover:bg-blue-500 ${data.active ? 'bg-blue-600' : 'bg-blue-100'}`}
                  style={{ height: idx === monthlyData.length - 1 ? '100%' : data.height }} // Usando 100% para o mês atual para demo
                />
                <span className={`text-xs font-medium ${data.active ? 'text-blue-600' : 'text-slate-400'}`}>
                  {data.month}
                </span>
              </div>
            ))}
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
          <h3 className="font-bold text-slate-800 text-lg mb-6">Status dos Pedidos</h3>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold text-slate-800">{statusCounts['Concluído'] || 0}</span>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold text-slate-800">{statusCounts['Pendente'] || 0}</span>
              </div>
              <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                <div 
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold text-slate-800">{statusCounts['Cancelado'] || 0}</span>
              </div>
              <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                <div 
            ...
            <div className="pt-6 border-t border-slate-100 mt-4">
              <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
                <div>
                  <p className="text-xs text-slate-500 uppercase tracking-wide font-semibold">Previsão de Receita</p>
                  <p className="text-slate-400 text-xs mt-1">Baseado nos pedidos pendentes</p>
                </div>
